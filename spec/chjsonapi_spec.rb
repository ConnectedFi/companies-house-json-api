...
describe ChJsonApi do
  ...
  describe "Company#profile" do
    ...
    it "searches for a company and return correct data " do
      ...
      expect(company["registered_office_address"]["address_line_1"]).to eq "Cms Cameron Mckenna Llp Cannon Place"
      ...
      expect(company["company_status"]).to eq "dissolved"
      expect(company["etag"]).to_not be_nil
      expect(company["has_charges"]).to eq true
      expect(company["can_file"]).to eq false
    end
  ...
  describe "Company#registered_address" do
    ...
    it "searches for the registered office and return correct data " do
      ...
      expect(reg_off["address_line_1"]).to eq "Cms Cameron Mckenna Llp Cannon Place"
    end
  ...
  describe "Company#officers" do
    ...
    it "searches for the officers and return correct data " do
      ...
      expect(off["active_count"]).to eq(0)
      expect(off["resigned_count"]).to eq(49)

      expect(off["items"][0]["address"]["premises"]).to eq "Cms Cameron Mckenna Llp"
      ...
      expect(off["items"][0]["appointed_on"]).to eq "2016-09-13"
      expect(off["items"][0]["name"]).to eq "PRINGLE, Martin"
    end
    ...
    it "returns an empty object if the company number does not exist" do
      expect { ChJsonApi::Company.officers company_number: "99999999" }.to raise_exception(Exception)
    end
  ...
  describe "Company#filing_list" do
    ...
    it "accepts all parameters and use them on the query" do
      ...
      expect(off["items"].count).to be 2
      expect(off["items_per_page"]).to be 2
      expect(off["items"][0]["category"]).to eq "resolution"
    end
  end
